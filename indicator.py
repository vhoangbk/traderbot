...
SOCKET = "wss://stream.binance.com:9443/ws/adausdt@kline_1h"
...
def on_message(ws, message):
    ...
    if is_candle_closed:
        ...
        if last_morning_star != 0:
            send_message('Morning star: {}'.format(close))


        hammer = talib.CDLHAMMER(np_opens, np_highs, np_lows, np_closes)
        last_hammer = hammer[-1]
        if last_hammer != 0:
            send_message('Hammer: {}'.format(close))
    
        hanging_man = talib.CDLHANGINGMAN(np_opens, np_highs, np_lows, np_closes)
        last_hanging_man = hanging_man[-1]
        if last_hanging_man != 0:
            send_message('Hanging Man: {}'.format(close))

        harami = talib.CDLHARAMI(np_opens, np_highs, np_lows, np_closes)
        last_harami = harami[-1]
        if last_harami != 0:
            send_message('Harami: {}'.format(close))
...
send_message("Indicator local bot stared!")   
